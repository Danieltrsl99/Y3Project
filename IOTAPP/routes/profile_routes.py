...
from routes.tuya_routes import list_devices  # Corrected import path
from routes.tuya_routes import control_device  # Import the Tuya control function
from tuya_utils import turn_on, turn_off, set_brightness, set_color  # Import the required functions
...
def profile():
    if 'user_id' in session:
        ...
        if user:  # Ensure the user exists in the database
...
def dashboard():
    if 'user_id' in session:
        ...
        if user:
            try:
                # Fetch devices from the Tuya API
                tuya_devices = list_devices()
                tuya_device_ids = {device['id'] for device in tuya_devices}

                # Fetch devices assigned to the user from the database
                ...
                # Filter devices that exist in the Tuya API
                user_devices = [
                ...
                devices = [
                    {
                        ...
                        "name": device.name,
                        "current_color": "#ffffff",  # Replace with actual color from the API
                        "current_brightness": 100,  # Replace with actual brightness from the API
                    }
...
@dash_bp.route('/claim_device', methods=['POST'])
def claim_device():
    if 'user_id' in session:
        ...
        if existing_device:
            flash('Device is already claimed.')
        else:
            # Insert the device into the database and assign it to the user
            db.session.execute(
            ...
            db.session.commit()
            flash('Device claimed successfully.')
